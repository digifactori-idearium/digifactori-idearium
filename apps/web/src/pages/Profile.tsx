...
import { followUser, getFollowers, getFollowing, getProfile } from "@/services/profile.service";
...
const ProfilePage: React.FC = () => {
  ...
  const [followers, setFollowers] = useState<{pseudo: string, avatar: string|null}[]>([]);
  const [showFollowers, setShowFollowers] = useState(false);
  const [following, setFollowing] = useState<{pseudo: string, avatar: string|null}[]>([]);
  ...
  return <div>
        ...
        onClick={() => {
            followUser(profile!.userId)
                ...
                if(profile?.followers.some(f => f == user?.id)) {
                    ...
                    setProfile({...profile, followers: profile.followers.filter(f => f != user?.id)});
                } else {
                    setFollowers([...followers, {pseudo: profile!.pseudo, avatar: profile!.avatar}]);
                    if(profile) {
                        setProfile({
                            ...profile,
    ...
    >
        {profile?.followers.some(f => f === user?.id) ? 'Unfollow' : 'Follow'} {profile?.pseudo}
    </SuperButton>}
    <div>
        Followers:
    {showFollowers && followers.map((follower, index) => (
        <div key={index}>
            <p>{follower.pseudo}</p>
            <Avatar className="h-14 w-14 border-2 border-white/20 shadow-sm shrink-0">
                <AvatarImage
    ...
    <div>
    Following:
    {showFollowing && following.map((followed, index) => (
        <div key={index}>
            <p>{followed.pseudo}</p>
            <Avatar className="h-14 w-14 border-2 border-white/20 shadow-sm shrink-0">
                <AvatarImage
