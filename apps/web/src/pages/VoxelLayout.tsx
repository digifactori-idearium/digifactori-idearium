  ...
  useEffect(() => {
    ...
    const loadModel = async () => {
      try {
        ...
        const fileKey = response.data.model; // Now this is the storage key (string)

        setModelName(response.data.name);

        // If fileKey is a string, fetch the actual model data from storage
        if (typeof fileKey === 'string') {
          const voxelData = await fetchVoxelModelFromStorage(fileKey);
          setVoxels(voxelData);
